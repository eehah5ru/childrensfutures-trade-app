...
(defn about-page []
  [outer-paper
   #_{:paper-props {:style {:padding-top "20px"}}}
   [:h1 "What kind of exchange market is it?"]
   [:p "The web application "
    [:a {:href "http://myfutures.trade" :target "blank"} "myfutures.trade"]
    (gstring/unescapeEntities " allows you to make transactions by trading future goals and dreams. This is a decentralized, speculative financial stock exchange that is based on a blockchain-technology called ")
    [:a {:href "http://ethereum.org" :target "blank"} "ethereum"] (gstring/unescapeEntities ". It is created to play with futures &mdash; investing into the events that have not yet occurred.")]
   ...
   [:p "Be available starting from 8 February!"]
